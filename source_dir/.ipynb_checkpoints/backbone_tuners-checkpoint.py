...
import torch
import transformers
from custom_loss import CosineEmbeddingLoss
from custom_loss import cosine_similarity
import pandas as pd
...
class MaskedLanguageModelingModel(pl.LightningModule):
    ...
    def __init__(self, embedding_backbone, learning_rate=1e-3, loss=torch.nn.CrossEntropyLoss()):
        ...
        super().__init__()
        self.model = embedding_backbone
        self.fc = torch.nn.Linear(self.embedding_backbone.config.hidden_size,
                                  self.embedding_backbone.config.vocab_size)
